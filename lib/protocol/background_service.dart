...
Stream<RecordingStateChangedEvent> get onRecordingStateChanged => _client.onEvent.where((event) => event.name == 'BackgroundService.recordingStateChanged')
...
Stream<BackgroundServiceEvent> get onBackgroundServiceEventReceived => _client.onEvent.where((event) => event.name == 'BackgroundService.backgroundServiceEventReceived')
.map((event) => BackgroundServiceEvent.fromJson(event.parameters['backgroundServiceEvent'] as Map<String, dynamic>))
;
...
Future<void> startObserving(
 ...
 await _client.send('BackgroundService.startObserving', {'service' : service,});
}
...
Future<void> setRecording(
  bool shouldRecord,  ServiceName service
) async {
 await _client.send('BackgroundService.setRecording', {'shouldRecord' : shouldRecord,'service' : service,});
}
...
RecordingStateChangedEvent({required this.isRecording,required this.service});
...
@override
bool operator ==(other) => (other is ServiceName && other.value == value) || value == other;
...
BackgroundServiceEvent({required this.timestamp,required this.origin,required this.serviceWorkerRegistrationId,required this.service,required this.eventName,required this.instanceId,required this.eventMetadata});
...
serviceWorkerRegistrationId:  service_worker.RegistrationID.fromJson(json['serviceWorkerRegistrationId'] as String),
service:  ServiceName.fromJson(json['service'] as String),
eventName:  json['eventName'] as String,
instanceId:  json['instanceId'] as String,
eventMetadata:  (json['eventMetadata'] as List).map((e) => EventMetadata.fromJson(e as Map<String, dynamic>)).toList(),
);
}
