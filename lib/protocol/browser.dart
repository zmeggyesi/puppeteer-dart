...
{  target.TargetID? browserContextId}
 ...
 await _client.send('Browser.setPermission', {'origin' : origin,'permission' : permission,'setting' : setting,if (browserContextId != null)'browserContextId' : browserContextId,});
}
...
Future<void> grantPermissions(
  String origin,  List<PermissionType> permissions
,
{  target.BrowserContextID? browserContextId}
) async {
 await _client.send('Browser.grantPermissions', {'origin' : origin,'permissions' : [...permissions],if (browserContextId != null)'browserContextId' : browserContextId,});
}
...
{  target.BrowserContextID? browserContextId}
 ...
 await _client.send('Browser.resetPermissions', {if (browserContextId != null)'browserContextId' : browserContextId,});
}
...
Future<List<String>> getBrowserCommandLine(
...
{  String? query,  bool? delta}
) async {
var result =  await _client.send('Browser.getHistograms', {if (query != null)'query' : query,if (delta != null)'delta' : delta,});
return (result['histograms'] as List).map((e) => Histogram.fromJson(e as Map<String, dynamic>)).toList();
}
...
var result =  await _client.send('Browser.getHistogram', {'name' : name,if (delta != null)'delta' : delta,});
...
var result =  await _client.send('Browser.getWindowBounds', {'windowId' : windowId,});
...
var result =  await _client.send('Browser.getWindowForTarget', {if (targetId != null)'targetId' : targetId,});
...
Future<void> setWindowBounds(
 ...
 await _client.send('Browser.setWindowBounds', {'windowId' : windowId,'bounds' : bounds,});
}
...
{  String? badgeLabel,  String? image}
) async {
 await _client.send('Browser.setDockTile', {if (badgeLabel != null)'badgeLabel' : badgeLabel,if (image != null)'image' : image,});
}
...
GetVersionResult({required this.protocolVersion,required this.product,required this.revision,required this.userAgent,required this.jsVersion});
...
@override
bool operator ==(other) => (other is WindowID && other.value == value) || value == other;
...
@override
bool operator ==(other) => (other is WindowState && other.value == value) || value == other;
...
windowState:  json.containsKey('windowState') ? WindowState.fromJson(json['windowState'] as String) : null,
);
}
...
static const periodicBackgroundSync = PermissionType._('periodicBackgroundSync');
static const protectedMediaIdentifier = PermissionType._('protectedMediaIdentifier');
...
@override
bool operator ==(other) => (other is PermissionType && other.value == value) || value == other;
...
@override
bool operator ==(other) => (other is PermissionSetting && other.value == value) || value == other;
...
PermissionDescriptor({required this.name,this.sysex,this.userVisibleOnly,this.type});
...
userVisibleOnly:  json.containsKey('userVisibleOnly') ? json['userVisibleOnly'] as bool : null,
...
Histogram({required this.name,required this.sum,required this.count,required this.buckets});
