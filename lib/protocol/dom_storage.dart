...
Stream<DomStorageItemAddedEvent> get onDomStorageItemAdded => _client.onEvent.where((event) => event.name == 'DOMStorage.domStorageItemAdded')
.map((event) => DomStorageItemAddedEvent.fromJson(event.parameters))
;



Stream<DomStorageItemRemovedEvent> get onDomStorageItemRemoved => _client.onEvent.where((event) => event.name == 'DOMStorage.domStorageItemRemoved')
.map((event) => DomStorageItemRemovedEvent.fromJson(event.parameters))
;



Stream<DomStorageItemUpdatedEvent> get onDomStorageItemUpdated => _client.onEvent.where((event) => event.name == 'DOMStorage.domStorageItemUpdated')
.map((event) => DomStorageItemUpdatedEvent.fromJson(event.parameters))
;



Stream<StorageId> get onDomStorageItemsCleared => _client.onEvent.where((event) => event.name == 'DOMStorage.domStorageItemsCleared')
.map((event) => StorageId.fromJson(event.parameters['storageId'] as Map<String, dynamic>))
;
...
var result =  await _client.send('DOMStorage.getDOMStorageItems', {'storageId' : storageId,});
return (result['entries'] as List).map((e) => Item.fromJson(e as List<String>)).toList();
}


Future<void> removeDOMStorageItem(
  StorageId storageId,  String key
) async {
 await _client.send('DOMStorage.removeDOMStorageItem', {'storageId' : storageId,'key' : key,});
}
...
Future<void> setDOMStorageItem(
 ...
 await _client.send('DOMStorage.setDOMStorageItem', {'storageId' : storageId,'key' : key,'value' : value,});
}
...
DomStorageItemAddedEvent({required this.storageId,required this.key,required this.newValue});
...
DomStorageItemUpdatedEvent({required this.storageId,required this.key,required this.oldValue,required this.newValue});
...
@override
bool operator ==(other) => (other is Item && other.value == value) || value == other;
