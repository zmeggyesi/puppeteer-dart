...
Stream<String> get onAddHeapSnapshotChunk => _client.onEvent.where((event) => event.name == 'HeapProfiler.addHeapSnapshotChunk')
.map((event) => event.parameters['chunk'] as String)
;


  /// If heap objects tracking has been started then backend may send update for one or more fragments
Stream<List<int>> get onHeapStatsUpdate => _client.onEvent.where((event) => event.name == 'HeapProfiler.heapStatsUpdate')
.map((event) => (event.parameters['statsUpdate'] as List).map((e) => e as int).toList())
;
...
Stream<LastSeenObjectIdEvent> get onLastSeenObjectId => _client.onEvent.where((event) => event.name == 'HeapProfiler.lastSeenObjectId')
.map((event) => LastSeenObjectIdEvent.fromJson(event.parameters))
;



Stream<ReportHeapSnapshotProgressEvent> get onReportHeapSnapshotProgress => _client.onEvent.where((event) => event.name == 'HeapProfiler.reportHeapSnapshotProgress')
.map((event) => ReportHeapSnapshotProgressEvent.fromJson(event.parameters))
;



Stream get onResetProfiles => _client.onEvent.where((event) => event.name == 'HeapProfiler.resetProfiles')
;
...
Future<void> addInspectedHeapObject(
 ...
 await _client.send('HeapProfiler.addInspectedHeapObject', {'heapObjectId' : heapObjectId,});
}
...
var result =  await _client.send('HeapProfiler.getHeapObjectId', {'objectId' : objectId,});
return HeapSnapshotObjectId.fromJson(result['heapSnapshotObjectId'] as String);
}
...
var result =  await _client.send('HeapProfiler.getObjectByHeapObjectId', {'objectId' : objectId,if (objectGroup != null)'objectGroup' : objectGroup,});
return runtime.RemoteObject.fromJson(result['result'] as Map<String, dynamic>);
}


  /// Returns: Return the sampling profile being collected.
Future<SamplingHeapProfile> getSamplingProfile(

) async {
var result =  await _client.send('HeapProfiler.getSamplingProfile');
return SamplingHeapProfile.fromJson(result['profile'] as Map<String, dynamic>);
}
...
{  num? samplingInterval}
 ...
 await _client.send('HeapProfiler.startSampling', {if (samplingInterval != null)'samplingInterval' : samplingInterval,});
}
...
{  bool? trackAllocations}
 ...
 await _client.send('HeapProfiler.startTrackingHeapObjects', {if (trackAllocations != null)'trackAllocations' : trackAllocations,});
}
...
{  bool? reportProgress}
 ...
 await _client.send('HeapProfiler.stopTrackingHeapObjects', {if (reportProgress != null)'reportProgress' : reportProgress,});
}
...
{  bool? reportProgress}
 ...
 await _client.send('HeapProfiler.takeHeapSnapshot', {if (reportProgress != null)'reportProgress' : reportProgress,});
}
...
factory HeapSnapshotObjectId.fromJson(String value) => HeapSnapshotObjectId(value);

String toJson() => value;

@override
bool operator ==(other) => (other is HeapSnapshotObjectId && other.value == value) || value == other;
...
SamplingHeapProfileNode({required this.callFrame,required this.selfSize,required this.id,required this.children});
...
id:  json['id'] as int,
children:  (json['children'] as List).map((e) => SamplingHeapProfileNode.fromJson(e as Map<String, dynamic>)).toList(),
);
}
...
SamplingHeapProfileSample({required this.size,required this.nodeId,required this.ordinal});
...
head:  SamplingHeapProfileNode.fromJson(json['head'] as Map<String, dynamic>),
samples:  (json['samples'] as List).map((e) => SamplingHeapProfileSample.fromJson(e as Map<String, dynamic>)).toList(),
);
}
