...
Future<GetDOMCountersResult> getDOMCounters(
...
Future<void> setPressureNotificationsSuppressed(
  bool suppressed
) async {
 await _client.send('Memory.setPressureNotificationsSuppressed', {'suppressed' : suppressed,});
}

  /// Simulate a memory pressure notification in all processes.
  /// [level] Memory pressure level of the notification.
Future<void> simulatePressureNotification(
  PressureLevel level
) async {
...
{  int? samplingInterval,  bool? suppressRandomness}
 ...
 await _client.send('Memory.startSampling', {if (samplingInterval != null)'samplingInterval' : samplingInterval,if (suppressRandomness != null)'suppressRandomness' : suppressRandomness,});
}
...
GetDOMCountersResult({required this.documents,required this.nodes,required this.jsEventListeners});
...
@override
bool operator ==(other) => (other is PressureLevel && other.value == value) || value == other;
...
SamplingProfileNode({required this.size,required this.total,required this.stack});
...
samples:  (json['samples'] as List).map((e) => SamplingProfileNode.fromJson(e as Map<String, dynamic>)).toList(),
modules:  (json['modules'] as List).map((e) => Module.fromJson(e as Map<String, dynamic>)).toList(),
);
}
...
Module({required this.name,required this.uuid,required this.baseAddress,required this.size});
