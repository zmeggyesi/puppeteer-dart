...
var result =  await _client.send('WebAuthn.addVirtualAuthenticator', {'options' : options,});
...
Future<void> removeVirtualAuthenticator(
 ...
 await _client.send('WebAuthn.removeVirtualAuthenticator', {'authenticatorId' : authenticatorId,});
}
...
Future<void> addCredential(
 ...
 await _client.send('WebAuthn.addCredential', {'authenticatorId' : authenticatorId,'credential' : credential,});
}
...
var result =  await _client.send('WebAuthn.getCredential', {'authenticatorId' : authenticatorId,'credentialId' : credentialId,});
...
var result =  await _client.send('WebAuthn.getCredentials', {'authenticatorId' : authenticatorId,});
return (result['credentials'] as List).map((e) => Credential.fromJson(e as Map<String, dynamic>)).toList();
}
...
Future<void> removeCredential(
 ...
 await _client.send('WebAuthn.removeCredential', {'authenticatorId' : authenticatorId,'credentialId' : credentialId,});
}
...
Future<void> clearCredentials(
 ...
 await _client.send('WebAuthn.clearCredentials', {'authenticatorId' : authenticatorId,});
}

  /// Sets whether User Verification succeeds or fails for an authenticator.
  /// The default is true.
Future<void> setUserVerified(
  AuthenticatorId authenticatorId,  bool isUserVerified
) async {
 await _client.send('WebAuthn.setUserVerified', {'authenticatorId' : authenticatorId,'isUserVerified' : isUserVerified,});
}
...
@override
bool operator ==(other) => (other is AuthenticatorId && other.value == value) || value == other;
...
@override
bool operator ==(other) => (other is AuthenticatorProtocol && other.value == value) || value == other;
...
@override
bool operator ==(other) => (other is AuthenticatorTransport && other.value == value) || value == other;
...
VirtualAuthenticatorOptions({required this.protocol,required this.transport,required this.hasResidentKey,required this.hasUserVerification,this.automaticPresenceSimulation});
...
automaticPresenceSimulation:  json.containsKey('automaticPresenceSimulation') ? json['automaticPresenceSimulation'] as bool : null,
);
}
...
Credential({required this.credentialId,required this.isResidentCredential,this.rpId,required this.privateKey,this.userHandle,required this.signCount});
...
userHandle:  json.containsKey('userHandle') ? json['userHandle'] as String : null,
